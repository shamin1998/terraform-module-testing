...
class TelemetryDataTransferLogger:
    ...
    def init_log(self,run_id,blobs,s3_bucket_dest_paths):
        '''
        ...
        Returns : 
         - run_id : An ID specifying a pipeline 'run', to help track the processing of specific Azure blobs through the pipeline
        '''
        ...
        item = {
            ...
            'run_id' : run_id,
            self.input_key : ','.join(blobs),
            's3_bucket_name' : self.s3_bucket_name,
            'output_path' : ','.join(s3_bucket_dest_paths),
            'adapter_state' : 'PENDING',
            'description' : f"Initializing transfer..."
            }
    ...
    def log_transfer(self,run_id,blobs,s3_bucket_dest_paths):
        '''
